"""
PowerShell-Specific Code Testing Agent

This agent specializes in running and analyzing PowerShell tests.
"""
...
class PowerShellCodeTesterAgent(CodeTesterBase):
    """
    Agent specialized for PowerShell code testing.
    
    Features:
    - Pester, PSScriptAnalyzer framework support
    ...
    def __init__(
        self,
        name: str = "code_tester_powershell",
        description: str = "PowerShell-specific code testing agent",
        **kwargs
    ):
        super().__init__(
            name=name,
            description=description,
            supported_frameworks=['Pester', 'PSScriptAnalyzer'],
            **kwargs
        )
    
    def _detect_test_framework(self, test_info: Dict[str, Any]) -> Optional[str]:
        """Detect PowerShell test framework."""
    ...
    def _build_test_command(
        ...
        """Build PowerShell test execution command."""
        if framework == 'Pester':
            cmd = ['Invoke-Pester']
            if test_path:
